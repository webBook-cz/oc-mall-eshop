...
class Orders extends Controller
{
    ...
    public function show()
    {
        ...
        $this->vars['shipping_address'] = Address::where('id', $order->shipping_address['id'])->first();
        $this->vars['billing_address'] = Address::where('id', $order->billing_address['id'])->first();
        $this->vars['money']        = app(Money::class);
    ...
    public function onSendOrders()
    {
        $sentStatus =  Db::table('webbook_mallextensions_statuses_codes')->where('code', 'sent')->first();
        $readyToShip =  Db::table('webbook_mallextensions_statuses_codes')->where('code', 'ready_to_ship')->first();

        if(Order::where('order_state_id', $readyToShip->code_id)->where('package_id', null)->where('is_sent', null)->count() == 0) {
            \Flash::error(Lang::get('webbook.mallextensions::lang.plugin.messages.no_orders_to_send'));
            return \Redirect::refresh();
        }

        $orders = Order::where('order_state_id', $readyToShip->code_id)->where('package_id', null)->where('is_sent', null)->get();

        foreach ($orders as $order) {
            $order->order_state_id = $sentStatus->code_id;
            $order->save();
        } 

        \Flash::success(Lang::get('webbook.mallextensions::lang.plugin.messages.orders_were_sent_successfully'));
